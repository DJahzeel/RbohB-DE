...
from download_data.check_download_files import (
    check_paired_files, 
    check_single_files, 
    save_sample_sheet
)
...
def process_runs(paired_path, single_path, paired_script, single_script, fq_output_dir, processing_prefix, root, paired_df, single_df):
    ...
    if paired_exists and not single_exists:
        ...
        submit_qsub_job(f"{processing_prefix}_paired_download", paired_script, paired_path, fq_output_dir, wait_for=True)
    ...
    if single_exists and not paired_exists:
        ...
        submit_qsub_job(f"{processing_prefix}_single_download", single_script, single_path, fq_output_dir, wait_for=True)
    ...
    if paired_exists and single_exists:
        ...
        verify_paired= check_paired_files(paired_df['SRR'], fq_output_dir)
        ...
        if all_verified_rows:
            sample_sheet_path = save_sample_sheet(all_verified_rows, root, processing_prefix)
...
def main():
    ...
    bpdir = base_project_root / "data"  / args.bioproject
    sra_files_dir = bpdir / "sra_files"
    gds_files_dir = bpdir / "gds_files"

    paired_script = base_project_root / "src" / "fasterqd_bash" /"sra_paired.jdl"
    single_script = base_project_root / "src" / "fasterqd_bash" /"sra_single.jdl"
    ...
    try:
        fastq_out_dir = fastq_directory(base_project_root, args.bioproject)
    ...
    if args.type == "GEO":
        ...
        for gse in gse_str.split(","):
            ...
            output_path = runends_directory(base_project_root, args.bioproject)
            paired_path, single_path = save_run_ends(paired, single, output_path, gse)
            process_runs(paired_path, single_path, paired_script, single_script, fastq_out_dir,gse, base_project_root, paired, single)
    ...
    if args.type == "GSM":
        ...
        paired, single = run_end(sra_subset)
        output_path = runends_directory(base_project_root, args.bioproject)
        paired_path, single_path = save_run_ends(paired, single,output_path, proccesing_prefix)
        process_runs(paired_path, single_path, paired_script, single_script, fastq_out_dir, proccesing_prefix,base_project_root, paired, single)
        ...
        process_runs(paired_path, single_path, paired_script, single_script, fastq_out_dir, args.bioproject, base_project_root, paired, single)
